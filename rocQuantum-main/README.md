...
| Top-level `rocq` | Canonical runtime path with native execute/sample/observe wiring, duplicate-combined Pauli expectations, coefficient-preserving composite observable sums, numeric identity constants, CUDA-Q-style `rocq.spin.x/y/z/i` Pauli factories, Pauli sum/product terms in operator arithmetic, and density-matrix correctness fallback for dense Hermitian / full-state CSR observables |
