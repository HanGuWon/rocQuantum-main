...
- PennyLane expectation sums that mix Pauli-representable terms with `qml.Hermitian` or `qml.SparseHamiltonian` now split into native component readouts for analytic expval and compatible `batch_execute`; dense Hermitian+Pauli mixed variance can use dense matrix moment hooks for small target sets; sparse mixed variance uses PennyLane's CSR observable representation with native sparse moments; and the same heterogeneous Pauli/dense/sparse term lists can enter native adjoint payloads.
