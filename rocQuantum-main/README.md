...
- The self-hosted ROCm runtime workflow builds native Python bindings and runs `scripts/native_framework_smoke.py`, a Bell-state smoke check through `rocquantum_bind`, PennyLane, Qiskit, and Cirq. It passes `--require-native-rocm-evidence`, uploads both `native-framework-smoke.log` and machine-readable `native-framework-smoke.json`, and records `native_rocm_evidence` / `evidence_kind` from the actual `/dev/kfd` device probe, so local source/mock tests, assumed-device runs, or locally skipped smoke JSON do not prove AMD GPU execution.
