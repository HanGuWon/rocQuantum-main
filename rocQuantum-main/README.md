...
- Qiskit, PennyLane, and Cirq adapters now prefer `QuantumSimulator.measure()` for sampling, and shared framework dispatch rejects bool, string, complex, and non-finite gate parameters before native single or batched gate calls; host-side fallback paths still remain where needed for older bindings that do not expose `measure`.
