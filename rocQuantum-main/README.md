...
- `rocquantum_bind.QuantumSimulator` can allocate `batch_size > 1` local state batches, read one state slice or the full `(batch_size, 2**num_qubits)` host array, apply batch-specific RX/RY/RZ angles, and return native batch-major probability matrices and Pauli-string expectation vectors. Qiskit/PennyLane helper code can combine batched Pauli expectations for already-prepared batched states, but the adapters do not yet automatically route broadcasted framework workloads through this batch surface. `python/rocq/api.py::Circuit` exposes batched state readback.
