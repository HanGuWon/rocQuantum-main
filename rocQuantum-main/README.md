...
- Qiskit, PennyLane, and Cirq adapters now prefer `QuantumSimulator.measure()` for sampling, shared framework dispatch rejects bool, string, complex, and non-finite gate parameters before native single or batched gate calls, and public native `QuantumSimulator` gate dispatch rejects non-finite single/batched angle parameters before HIP kernels are invoked; host-side fallback paths still remain where needed for older bindings that do not expose `measure`.
