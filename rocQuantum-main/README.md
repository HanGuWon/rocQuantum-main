...
| --- | --- |
| `hipStateVec` | Real and useful, with local batched state allocation/readback, batch-specific RX/RY/RZ, batched probabilities, and batched Pauli expectations exposed through `QuantumSimulator`; not yet fully surfaced through every framework adapter |
...
- `rocquantum_bind.QuantumSimulator` can allocate `batch_size > 1` local state batches, read one state slice or the full `(batch_size, 2**num_qubits)` host array, apply batch-specific RX/RY/RZ angles, and return native batch-major probability matrices and Pauli-string expectation vectors. Qiskit Estimator can route simple Pauli-observable parameter batches through this batch surface; PennyLane helper code can combine batched Pauli expectations for already-prepared batched states. Broader broadcasted framework workloads still need more adapter coverage. `python/rocq/api.py::Circuit` exposes batched state readback.
