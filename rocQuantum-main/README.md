...
- The self-hosted ROCm runtime workflow builds native Python bindings and runs `scripts/native_framework_smoke.py`, a Bell-state smoke check through `rocquantum_bind`, PennyLane, Qiskit, and Cirq. It uploads both `native-framework-smoke.log` and machine-readable `native-framework-smoke.json`; local source/mock tests or locally skipped smoke JSON do not prove AMD GPU execution.
