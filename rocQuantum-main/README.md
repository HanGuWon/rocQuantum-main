...
- `QuantumSimulator.apply_sparse_matrix()` / `ApplySparseMatrix()` expose a CSR sparse-operation hook through the public binding. The current implementation avoids dense sparse-operator materialization but applies the CSR operation through host statevector readback/upload; a GPU-resident sparse gate-application kernel remains a performance gap.
