...
- TensorNet supports the build's compiled complex dtype (`C64` by default, `C128` in `ROCQ_PRECISION_DOUBLE` builds); METIS/KAHYPAR pathfinders and runtime slicing report unsupported unless compiled in, the Python binding expands TensorNet status codes into messages that point callers to `get_tensornet_capabilities()`, and `memory_limit_bytes` / `num_slices` emit a `RuntimeWarning` because they currently influence planning cost only rather than cuTensorNet-style sliced execution.
