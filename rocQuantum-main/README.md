...
- TensorNet supports the build's compiled complex dtype (`C64` by default, `C128` in `ROCQ_PRECISION_DOUBLE` builds); METIS/KAHYPAR pathfinders and runtime slicing report unsupported unless compiled in, and the Python binding expands TensorNet status codes into messages that point callers to `get_tensornet_capabilities()`.
