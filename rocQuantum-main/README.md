...
- PennyLane expectation sums that mix Pauli-representable terms with `qml.Hermitian` or `qml.SparseHamiltonian` now split into native component readouts for analytic expval and compatible `batch_execute`; the same heterogeneous Pauli/dense/sparse term lists can enter native adjoint payloads. Mixed dense/sparse variance cross terms still fall back because they need explicit product lowering.
