...
- `QuantumSimulator.apply_sparse_matrix()` / `ApplySparseMatrix()` expose a CSR sparse-operation hook through the public binding and route local single-state or batched state vectors through `rocsvApplySparseMatrix`, avoiding dense sparse-operator materialization and Python statevector readback. Distributed sparse apply and AMD GPU performance validation remain pending.
