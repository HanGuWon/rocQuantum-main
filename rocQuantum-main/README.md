...
- PennyLane compatible `batch_execute` normalizes scalar-multiple Pauli, dense Hermitian, and CSR `SparseHamiltonian` readout payloads inside the prepared batch cache, so duplicate base observables are evaluated once and coefficients are reapplied to means/moments without extra native hooks or statevector fallback.
