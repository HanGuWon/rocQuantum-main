...
- `rocquantum_bind.QuantumSimulator` can allocate `batch_size > 1` local state batches, read one state slice or the full `(batch_size, 2**num_qubits)` host array, apply batch-specific RX/RY/RZ and CRX/CRY/CRZ angles, and return native batch-major probability matrices and Pauli-string expectation vectors. Qiskit Estimator and PennyLane `batch_execute` can route simple Pauli-observable parameter batches through this batch surface; Qiskit Sampler and PennyLane also route simple batched probability readout through native batch probabilities. The shared runtime now exposes `measure_batch()` as the adapter-facing sampling hook, falling back to probability-based host draws until a binding-level batched sampler is available. Broader broadcasted framework workloads still need more adapter coverage. `python/rocq/api.py::Circuit` exposes batched state readback.
