...
| `rocqCompiler` | Partial source-level codegen path plus a narrow compile-and-execute MVP for qalloc/H/X/Y/Z/S/Sdg/T/Tdg/CNOT/CZ/SWAP/CCX/MCX/CSWAP/RX/RY/RZ/P/CRX/CRY/CRZ/CP; the default `rocquantum_bind` build now exposes a fail-fast compiler guard instead of unresolved MLIR symbols |
...
| Higher-level helpers | Experimental VQE objective/gradient, VQE-compatible MaxCut-style QAOA kernel/cost/solve helper that maximizes cut value via a negated-cost VQE objective, and 3-qubit repetition-code single/repeated-round helpers with syndrome/correction analysis |
...
- Qiskit, PennyLane, and Cirq adapters now prefer `QuantumSimulator.measure()` for sampling, shared framework dispatch rejects bool, string, complex, and non-finite gate parameters before native single or batched gate calls, and public native `QuantumSimulator` gate dispatch rejects non-finite single/batched angle parameters plus non-finite dense matrix/control-matrix/expectation payloads before HIP kernels or device uploads are invoked; host-side fallback paths still remain where needed for older bindings that do not expose `measure`.
- The self-hosted ROCm runtime workflow builds native Python bindings and runs `scripts/native_framework_smoke.py`, a Bell-state smoke check through `rocquantum_bind`, PennyLane, Qiskit, and Cirq. Local source/mock tests do not prove AMD GPU execution; use the uploaded `native-framework-smoke.log` artifact for ROCm E2E evidence.
