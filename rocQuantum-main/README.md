...
- PennyLane `ControlledSequence` now keeps adjoint phase-root bases (`Adjoint(S)` / `Adjoint(T)`) on native controlled-phase paths, including fixed blocks inside compatible `batch_execute`.
