...
- PennyLane general multi-control `qml.ctrl(...)` wrappers for single-qubit `PauliX` / `PauliY` / `PauliZ` / `Hadamard` / `S` / `T` bases are now accepted directly by the device: Pauli/Hadamard cases lower through `MCX`-backed decompositions, phase-root cases use the small controlled-matrix hook, open controls use exact `X` wrapping, and fixed compatible wrappers stay inside `batch_execute` instead of triggering PennyLane-expanded local matrix fallback.
