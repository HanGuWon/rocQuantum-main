...
- PennyLane `diff_method="adjoint"` now probes an optional binding-level `adjoint_jacobian` / `AdjointJacobian` hook for Pauli-term, dense Hermitian, and full-state CSR SparseHamiltonian observable payloads, skips Python statevector capture when that hook accepts the payload, and falls back cleanly when the hook reports an unsupported payload through `NotImplementedError` or ROCm-style status 5; the shipped ROCm backend still needs a real GPU-resident adjoint Jacobian implementation behind that hook.
