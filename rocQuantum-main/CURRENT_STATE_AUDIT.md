...
- PennyLane `diff_method="adjoint"` now probes a binding-level `adjoint_jacobian` / `AdjointJacobian` hook for scalar-parametric operation payloads plus Pauli-term, dense Hermitian, and full-state CSR SparseHamiltonian observable payloads, labels each operation's global trainable parameter indices and operation-local trainable parameter positions, lowers `Rot` / `CRot` into primitive rotation payloads, and skips Python statevector capture when that hook accepts the payload. The root binding now implements an exact adjoint path for trainable RX/RY/RZ/CRX/CRY/CRZ operations with Pauli-term, dense Hermitian, and full-state CSR sparse observables, and falls back cleanly for unsupported payloads; broader operation payloads, partial sparse adjoint payloads, and a GPU-resident adjoint Jacobian implementation remain pending.
