...
- Public `QuantumSimulator` now exposes Pauli, dense-matrix, batched dense-matrix, and sparse-CSR expectation helpers through `expectation_value()`, `expectation_pauli_string()`, `expectation_matrix()`, `expectation_matrix_batch()`, and `sparse_hamiltonian_moments()`, with root pybind coverage for framework adapters and shared-runtime statevector fallback for older dense/sparse expectation bindings.
