...
- PennyLane computational-basis `qml.Projector` expectations now expand to Pauli-Z product terms and use native Pauli-string expectations instead of mandatory statevector fallback; `qml.Hermitian` and scalar-scaled Hermitian expectation and variance can use dense `QuantumSimulator.expectation_matrix()` / `expectation_matrix_batch()` paths plus public `expectation_matrix_moments()` / `expectation_matrix_moments_batch()` moment helpers for supported target sizes, scalar identity Hermitian plus small diagonal Hermitian and CSR SparseHamiltonian matrices are folded into constant or Pauli-Z payloads for expval/variance/batch/adjoint paths without runtime readout, and shared-runtime statevector fallback avoids duplicate single-state and batched statevector reads for older bindings.
