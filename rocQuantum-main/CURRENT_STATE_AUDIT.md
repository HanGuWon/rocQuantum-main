...
- PennyLane analytic `qml.probs()` now prefers a shared `QuantumSimulator.probabilities()` runtime hook when a binding exposes it, while keeping the cached statevector fallback for current/legacy bindings.
