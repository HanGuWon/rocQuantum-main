...
- PennyLane mixed expectation sums combining Pauli-representable terms with `qml.Hermitian` or `qml.SparseHamiltonian` now decompose into native component readouts for analytic expval and compatible `batch_execute`, and explicit adjoint payloads can carry heterogeneous Pauli, dense-matrix, and CSR sparse terms in one observable. Mixed dense/sparse variance cross terms still fall back until product lowering is added.
