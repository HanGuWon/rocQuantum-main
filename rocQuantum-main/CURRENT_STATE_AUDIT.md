...
- PennyLane mixed expectation sums combining Pauli-representable terms with `qml.Hermitian` or `qml.SparseHamiltonian` now decompose into native component readouts for analytic expval and compatible `batch_execute`; dense Hermitian+Pauli mixed variance can use dense matrix moment hooks for small target sets; and explicit adjoint payloads can carry heterogeneous Pauli, dense-matrix, and CSR sparse terms in one observable. Sparse mixed variance cross terms still fall back until CSR product lowering is added.
