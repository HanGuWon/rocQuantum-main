...
- Mid-circuit measurement plus classical control flow is limited to simple Qiskit `if_test` / `if_else`, finite `for_loop`, bounded `while_loop`, and `switch_case` sampling trajectories; broader break/continue and classical-control semantics remain absent.
