...
- Qiskit native Estimator now treats dense scalar `Operator([[c]])` observables as constant expectation values for single and batched pubs, so those observables no longer trigger dense expectation hooks or statevector readback.
