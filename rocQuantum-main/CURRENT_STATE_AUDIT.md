...
- PennyLane computational-basis `qml.Projector` expectations now expand to Pauli-Z product terms and use native Pauli-string expectations instead of mandatory statevector fallback; `qml.Hermitian` and scalar-scaled Hermitian expectation and variance can use dense `QuantumSimulator.expectation_matrix()` / `expectation_matrix_batch()` paths plus public `expectation_matrix_moments()` / `expectation_matrix_moments_batch()` moment helpers for supported target sizes, scalar identity Hermitian and small diagonal Hermitian matrices are folded into constant or Pauli-Z payloads for expval/variance/batch/adjoint paths without runtime readout, scalar identity CSR SparseHamiltonian matrices are folded into constant Pauli payloads, and shared-runtime statevector fallback avoids duplicate single-state and batched statevector reads for older bindings.
- PennyLane `qml.SparseHamiltonian` and scalar-scaled SparseHamiltonian analytic expectation / variance now prefers `QuantumSimulator.sparse_hamiltonian_moments()` / `sparse_hamiltonian_moments_batch()`, which upload CSR buffers and call `rocsvGetSparseMatrixMoments` / `rocsvGetSparseMatrixMomentsBatch` for native HIP row-wise sparse moments reductions; scalar wrappers are absorbed into the CSR data before native dispatch, distributed sparse moments are explicit slow/debug host fallback only, and unsupported bindings now fall back through the shared runtime's statevector CSR moments path.
