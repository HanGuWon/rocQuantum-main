...
- PennyLane-expanded `qml.QROM` selected `Prod(BasisEmbedding, ...)` loaders now lower to grouped open-control `CNOT` / `MCX` target batches and native `CSWAP` swap networks instead of dense full-select matrices; positive-coefficient `qml.PrepSelPrep` selected Pauli products lower through partial `Select` native controlled-Pauli decompositions, and `qml.FABLE` is verified to follow PennyLane's native rotation/swap decomposition for covered shapes.
