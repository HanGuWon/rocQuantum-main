...
| Expectations | Native `hipStateVec` helpers exist; canonical `rocq.observe()` and legacy `Circuit.expval()` are wired for supported Pauli operators; local batched Pauli-string expectation is exposed through public and legacy binding surfaces; canonical `HermitianOperator` and PennyLane `qml.Hermitian` can use dense matrix expectation for supported target sizes; local batched dense-matrix expectation is exposed through public and legacy binding surfaces; canonical `SparseHamiltonianOperator` and PennyLane `qml.SparseHamiltonian` can use native CSR sparse moments for local single-state vectors | Broader arbitrary operator coverage, large dense matrices, distributed dense expectations, and batched/distributed sparse expectations remain limited |
