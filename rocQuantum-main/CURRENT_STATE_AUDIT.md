...
- Canonical `QuantumOperator` supports subtraction/negation, and the experimental MaxCut QAOA cost helper now includes the weighted identity offset in each `0.5 * w * (I - Zi Zj)` edge term instead of returning only the `-0.5 * w * ZiZj` component.
