...
- PennyLane full and partial `qml.Select` blocks now dispatch supported selected operations through native controlled gates or the small controlled-matrix hook instead of full-register dense select matrices, including batched selected single-qubit rotation angles; covered selected operations, fixed selected `BasisEmbedding`, simple selected basis/phase products, and fixed selected matrix operations also lower into primitive adjoint payloads around supported trainable rotations.
...
- PennyLane `diff_method="adjoint"` now probes a binding-level `adjoint_jacobian` / `AdjointJacobian` hook for scalar-parametric operation payloads plus Pauli-term, dense Hermitian, and full-state or targeted CSR SparseHamiltonian observable payloads, labels each operation's global trainable parameter indices, operation-local trainable parameter positions, and optional parameter-derivative scales, lowers `Rot` / `CRot` into primitive rotation payloads, maps `PhaseShift` / controlled-phase payloads to native `P` / `CP`, lowers covered `qml.ctrl(...)` wrappers for `RX` / `RY` / `RZ` / `PhaseShift` / `Rot` / `PauliX` / `PauliY` / `PauliZ` / `Hadamard` / `S` / `T` / `Adjoint(S)` / `Adjoint(T)` / `CPhaseShift00/01/10` / `SWAP` / `ISWAP` / `PSWAP` / `SISWAP` / `SQISW` / `ECR` / targetless `GlobalPhase` through primitive `RZ` / `CNOT` / `MCX` / `CR*` / `CP` / `CSWAP` payloads with open-control `X` wrapping, lowers fixed and scalar-parametric `ControlledSequence` controlled powers, lowers covered `Select` branches including fixed selected `BasisEmbedding`, simple selected basis/phase products, and fixed selected matrix operations, lowers decomposition-backed `MultiRZ` / `PauliRot` / fixed `DiagonalQubitUnitary` / fixed `SelectPauliRot` / `IsingXX/YY/ZZ/XY` / `SingleExcitation` plus/minus / `DoubleExcitation` plus/minus / `PSWAP` / `FermionicSWAP` / `OrbitalRotation` payloads, lowers fixed `QFT` / `BasisEmbedding` / `Permute` / `QubitSum` / `QubitCarry` / `GroverOperator` templates into primitive payloads, and skips Python statevector capture when that hook accepts the payload. The root binding now implements an exact adjoint path for trainable RX/RY/RZ/P/CRX/CRY/CRZ/CP operations with Pauli-term, dense Hermitian, full-state CSR sparse, targeted partial CSR sparse observables, fixed `DiagonalQubitUnitary` primitive phase-decomposition operation payloads, fixed `SelectPauliRot` primitive uniformly controlled rotation payloads, fixed and scalar-parametric `ControlledSequence` primitive controlled-power operation payloads, covered `Select` primitive payloads including fixed selected `BasisEmbedding`, simple selected basis/phase products, and fixed selected matrix operations, fixed `QFT` / `BasisEmbedding` / `Permute` / `QubitSum` / `QubitCarry` / `GroverOperator` template payloads, and fixed `QubitUnitary` / `ControlledQubitUnitary` / dense `BlockEncode` matrix operation payloads and sparse `BlockEncode` CSR operation payloads, including scaled decomposition parameters, and falls back cleanly for unsupported payloads; trainable matrix-parameter payloads, trainable diagonal or SelectPauliRot angle-array payloads, remaining trainable controlled-matrix or multi-target controlled-wrapper payloads, trainable selected BasisEmbedding arrays, unsupported selected product combinations, or trainable selected matrix payloads, and a GPU-resident adjoint Jacobian implementation remain pending.
