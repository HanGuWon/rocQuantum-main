...
6. Gate fusion exists in C++ and is used by the canonical `rocq` backend and legacy `python/rocq` flush path for same-target single-qubit spans plus narrow CNOT-adjacent spans; unsupported fusion inputs now fail rather than being silently dropped, but broader fusion patterns are still unfused.
