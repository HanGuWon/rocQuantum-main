...
5. Packaging and build surfaces are closer but still not one releasable product: root CMake now activates the `_rocq_hip_backend` owner in `python/rocq`, while `pyproject.toml`, compatibility `setup.py` files, and the canonical/legacy Python surfaces still need consolidation.
...
10. Higher-level CUDA-QX-style libraries now have an experimental VQE/QAOA/repetition-code subset, including public VQE energy evaluation, repeated-round repetition-code aggregation, and narrow syndrome readout-error mitigation, but this is still far from a serious supported CUDA-QX analogue.
