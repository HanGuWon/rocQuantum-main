...
class TestVqeSolverContract(unittest.TestCase):
    def test_solver_capabilities_expose_experimental_cudaqx_boundary(self):
        ...
        self.assertEqual(
            capability_data["execution_scope"]["quantum_runtime"],
            "canonical_rocq_observe_local_backends",
        )
        self.assertEqual(
            capability_data["execution_scope"]["classical_optimizer"],
            "host_python_optimizer_loop",
        )
        self.assertEqual(
            capability_data["execution_scope"]["gradients"],
            "host_parameter_shift_or_finite_difference",
        )
        self.assertEqual(
            capability_data["execution_scope"]["native_adjoint"],
            "unsupported",
        )
        self.assertEqual(
            capability_data["execution_scope"]["distributed_solver_execution"],
            "unsupported",
        )
        self.assertFalse(capability_data["hardware_evidence"]["probe_performed"])
        self.assertFalse(capability_data["hardware_evidence"]["capability_query_is_runtime_proof"])
    ...
    def test_vqe_rejects_non_operator_hamiltonian_before_backend_or_optimizer_use(self):
        from rocquantum.solvers.vqe_solver import Optimizer, VQE_Solver
        ...
        solver = VQE_Solver()
        invalid_hamiltonians = ("Z0", None, object(), 1.0)
        for hamiltonian in invalid_hamiltonians:
            with self.subTest(hamiltonian=hamiltonian):
                with mock.patch("rocquantum.solvers.vqe_solver.observe") as patched_observe:
                    with self.assertRaisesRegex(ValueError, "rocq.operator.QuantumOperator"):
...
class TestQecHelpers(unittest.TestCase):
    def test_qec_capabilities_expose_experimental_cudaqx_boundary(self):
        ...
        self.assertEqual(
            capability_data["execution_scope"]["quantum_runtime"],
            "canonical_rocq_sample_local_backends",
        )
        self.assertEqual(
            capability_data["execution_scope"]["feedback"],
            "sequential_sampled_classical_post_processing",
        )
        self.assertEqual(
            capability_data["execution_scope"]["mid_circuit_dynamic_feedback"],
            "unsupported",
        )
        self.assertEqual(
            capability_data["execution_scope"]["decoder_scope"],
            "lookup_table_repetition_code",
        )
        self.assertEqual(
            capability_data["execution_scope"]["distributed_qec_execution"],
            "unsupported",
        )
        self.assertFalse(capability_data["hardware_evidence"]["probe_performed"])
        self.assertFalse(capability_data["hardware_evidence"]["capability_query_is_runtime_proof"])
    ...
    def test_qec_experiment_validates_stabilizer_fragment_collection(self):
        ...
        with self.assertRaisesRegex(ValueError, "at least one circuit fragment"):
            experiment.run_single_round(FragmentCode([]), decoder, None, 5, [], shots=1)
        with self.assertRaisesRegex(ValueError, "at least one circuit fragment"):
            experiment.run_single_round(FragmentCode(iter(())), decoder, None, 5, [], shots=1)
        with self.assertRaisesRegex(ValueError, "sequence of circuit fragments"):
            experiment.run_single_round(
        ...
        with mock.patch(
            ...
            result = experiment.run_single_round(
                FragmentCode(fragment for fragment in ("fragment-0", "fragment-1")),
                generator_decoder,
                None,
                5,
                [3, 4],
