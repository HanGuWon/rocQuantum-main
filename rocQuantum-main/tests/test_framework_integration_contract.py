...
class TestFrameworkIntegrationContract(unittest.TestCase):
    def test_native_rocm_framework_smoke_is_source_defined_for_ci(self):
        ...
        self.assertIn("-m pybind11 --cmakedir", workflow)
        self.assertIn('-DPython3_EXECUTABLE="${VENV_PYTHON}"', workflow)
        self.assertIn('python -m venv --clear "${RUNNER_TEMP}/rocq-framework-venv"', workflow)
        self.assertNotIn("pip install --user", workflow)
        ...
        self.assertIn("pennylane>=0.45,<0.46", workflow)
        self.assertNotIn("pennylane>=0.38", workflow)
        self.assertIn("qiskit>=2.4,<3", workflow)
        self.assertIn("cirq-core>=1.0,<2", workflow)
    ...
    def test_shared_runtime_exposes_controlled_rotation_aliases(self):
        ...
        self.assertIn("SINGLE_QUBIT_GATES", source)
        self.assertIn("TWO_QUBIT_GATES", source)
        self.assertIn("PARAMETRIC_GATES", source)
        self.assertIn("from numbers import Integral", source)
        self.assertIn("def normalize_positive_integer", source)
        self.assertIn("def normalize_nonnegative_integer", source)
        self.assertIn("STATEVECTOR_MAX_QUBITS_BEFORE_SIZE_OVERFLOW = 60", source)
        self.assertIn("def statevector_dimension", source)
        ...
        self.assertIn("def validate_gate_signature", source)
        self.assertIn("not isinstance(target, Integral)", source)
        self.assertIn("def normalize_shots", source)
        self.assertIn('return normalize_positive_integer(shots, "shots")', source)
        self.assertIn('return normalize_nonnegative_integer(getter(), "Simulator qubit count")', source)
        self.assertIn('return normalize_positive_integer(getter(), "Simulator batch size")', source)
        self.assertIn("must be a positive integer", source)
        ...
        self.assertIn("def normalize_probability_result_vector", source)
        self.assertIn("def normalize_probability_result_matrix", source)
        self.assertIn("def probability_outcome_count", source)
        self.assertIn("return statevector_dimension(num_qubits)", source)
        ...
        self.assertIn("normalized_targets = validate_gate_signature", source)
        ...
        self.assertIn("def dense_expectation_matrix_for_targets", source)
        self.assertIn("def expectation_matrix(self, matrix", source)
        self.assertIn("def expectation_matrix_moments", source)
        self.assertIn("dense_expectation_matrix_for_targets(", source)
        self.assertIn("def set_statevector", source)
        self.assertIn("statevector_dimension(self.num_qubits()", source)
    ...
    def test_public_simulator_exposes_native_probabilities(self):
        ...
        self.assertIn("def _native_binding_unavailable", runtime)
        self.assertNotIn("def _native_probabilities_unavailable", runtime)
    ...
    def test_public_simulator_exposes_sparse_hamiltonian_moments(self):
        ...
        self.assertIn('pennylane_requires = ">=0.45,<0.46"', pennylane)
        ...
        self.assertIn("validated_state_info", low_level_bindings)
        self.assertIn("rocsvGetStateInfo", low_level_bindings)
        self.assertNotIn("infer_batch_size_from_state_buffer", low_level_bindings)
    ...
    def test_pennylane_sampling_prefers_native_measure(self):
        ...
        self.assertIn("parameter_lists_match,", source)
        self.assertNotIn("def _parameter_lists_match", source)
