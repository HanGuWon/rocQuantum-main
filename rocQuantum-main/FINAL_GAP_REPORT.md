...
- `hipDensityMat` exists, and now has single-qubit Kraus channels, density sampling that reduces measured-qubit marginals on GPU before host-side shot drawing, and native dense Hermitian expectation for up to four target qubits, but GPU-resident RNG/CDF sampling, optimized channel scheduling, CSR density observables, and broader descriptor coverage lag behind cuDensityMat expectations
