...
| Compiler-driven execution | PARTIAL | `compile_and_execute()` now executes a narrow qalloc/H/X/Y/Z/S/Sdg/T/CNOT/CZ/SWAP/CCX/MCX/CSWAP/RX/RY/RZ/CRX/CRY/CRZ MLIR subset; broad compiler/runtime parity is still absent |
...
- only a narrow mid-circuit measurement and classical-control story: Qiskit simple `if_test` / `if_else`, static `for_loop`, and `switch_case` sampling trajectories work, but parameterized-loop/while-loop/estimator/statevector dynamic semantics remain open
...
9. Integrations are still thin adapters, though PennyLane/Cirq/Qiskit sampling now prefers the native simulator `measure()` path where available, Qiskit simple `if_test` / `if_else`, static `for_loop`, and `switch_case` sampling can use state-collapsing `measure_qubit()` trajectories, PennyLane/Qiskit Pauli-observable paths use native expectation helpers where possible, PennyLane `qml.Hermitian` and supported Qiskit dense `Operator` observables can use native dense-matrix single/batch expectation paths for supported targets, Qiskit/PennyLane default multi-control gates reach native `MCX` / `CSWAP`, and PennyLane `SparseHamiltonian` can use native CSR moments, including local batch readouts, with a CSR statevector fallback.
