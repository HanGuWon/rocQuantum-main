...
- **Native Controlled Rotations**: Exposes Qiskit `crx`, `cry`, `crz`, and `cr` target operations, dispatching axis rotations to rocQuantum native controlled gates and lowering controlled `R(theta, phi)` through exact controlled-`U3` decompositions.
...
- The target includes `crx`, `cry`, `crz`, and `cr`; older bindings can still fall back through matrix application when available.
