...
- The target includes `crx`, `cry`, `crz`, fixed `ccrx` / `c3rx` / `ccry` / `c3ry` / `ccrz` / `c3rz`, and `cr`; older bindings can still fall back through matrix application when available.
