...
- `backend.run(..., sampling=False)` skips measurement and counts formatting for statevector-only workloads. Compatible circuit lists over the safe direct/batched gate subset, including fixed `PauliGate` / `unitary` operations, `rxx` / `ryy` / `rzz` / `rzx`, top-level/global-phase annotations, and `PauliEvolutionGate` sweeps with identity terms, share one batched simulator allocation and one batch-major statevector readback with per-circuit phase correction applied after readback; phase-gate sweeps use this path only when native `P` / `CP` batch dispatch is available.
