...
- **Basic Dynamic Sampling**: Executes simple Qiskit `if_test` / `if_else` conditioned blocks in `backend.run(..., sampling=True)` by using a state-collapsing `QuantumSimulator.measure_qubit()` trajectory per shot. Statevector and estimator outputs still reject dynamic circuits.
...
- Qiskit `if_test` / `if_else` blocks with tuple-style classical conditions can run in the same shot-trajectory sampling mode after prior measurements. Loop/switch/break/continue control-flow operations, statevector output, and estimator expectation for dynamic circuits are still rejected explicitly.
