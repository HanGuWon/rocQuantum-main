...
- **Native Open-Control Decomposition**: Dispatches direct open-control Qiskit controlled-X and single-control controlled-H operations through exact `x`-flip plus native `cx` / `mcx` / `ch` decompositions instead of dense matrices.
...
- Direct execution can also matrix-dispatch non-target unitary instructions with up to four qubits when Qiskit can provide a dense matrix. Open-control controlled-X and single-control controlled-H variants such as `ccx_o1` / `mcx_o5` / `ch_o0` prefer exact native decomposition; larger, non-unitary, or unsupported controlled-unitary instructions remain unsupported unless added explicitly.
