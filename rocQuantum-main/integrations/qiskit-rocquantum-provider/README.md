...
- Qiskit `if_test` / `if_else` blocks with tuple-style classical conditions, finite `for_loop` blocks with numeric loop-parameter binding, and `switch_case` blocks over classical bits/registers can run in the same shot-trajectory sampling mode after prior measurements. `while_loop`, break/continue control-flow operations, statevector output, and estimator expectation for dynamic circuits are still rejected explicitly.
