...
- **Native Open-Control Decomposition**: Dispatches direct open-control Qiskit controlled-X/Y/Z and single-control controlled-H operations through exact `x`-flip plus native `cx` / `mcx` / `cy` / `cz` / `ccz` / `ch` decompositions instead of dense matrices.
...
- **State Preparation Boundary**: Uploads full-wire initial `QuantumCircuit.prepare_state()` / `initialize()` vectors through `QuantumSimulator.set_statevector()` and keeps partial state preparation on the matrix fallback path; later `initialize()` is rejected because it requires non-unitary reset support.
...
- Qiskit `sx`, `sxdg`, `tdg`, `u`, `p`, `cp`, `rxx`, `ryy`, and `rzz` are target-visible but prefer exact native decomposition; statevector-producing `sx` / `sxdg` / `tdg` / `u` / `p` / `cp` runs include the required global phase, while sampling-only and estimator runs omit it because it cannot affect observed results.
- Direct execution can also matrix-dispatch non-target unitary instructions with up to four qubits when Qiskit can provide a dense matrix. Open-control controlled-X/Y/Z and single-control controlled-H variants such as `ccx_o1` / `mcx_o5` / `cy_o0` / `cz_o0` / `ccz_o1` / `ch_o0` prefer exact native decomposition; larger, non-unitary, or unsupported controlled-unitary instructions remain unsupported unless added explicitly.
