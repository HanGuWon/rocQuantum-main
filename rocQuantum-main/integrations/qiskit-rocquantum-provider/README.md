...
- `ccx`, general all-one-control `mcx`, and `cswap` are routed to native multi-control public simulator gates when available. `ch`, `cy`, `ccz`, `dcx`, `ecr`, `iswap`, `rccx`, `rcccx`, `MCPhaseGate`, and the controlled-U family `cs` / `csdg` / `csx` / `cu1` / `cu3` / `cu` use exact native decompositions.
