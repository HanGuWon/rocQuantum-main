...
- **Native Controlled-U Family Decomposition**: Dispatches Qiskit `cs`, `csdg`, `csx`, `cu1`, `cu3`, and `cu` through exact native `p` / `cp` / `crx` / `cx` / `u` decompositions instead of dense controlled matrices; `cu1` / `cu3` / `cu` parameter sweeps stay in the batched simulator path.
