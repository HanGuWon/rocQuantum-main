...
- **Native Expectations**: `RocQuantumProvider.estimate_expectation()` evaluates Qiskit `SparsePauliOp`/`Pauli` observables through the rocQuantum Pauli-string expectation path and full-circuit dense `Operator` observables through the native dense-matrix expectation path. The shared runtime also exposes batched dense-matrix expectations for adapter paths that can supply dense observable plans directly.
...
- `RocQuantumEstimator` groups broadcast entries by parameter index, so a pub with one bound circuit and many observables reuses the prepared simulator state instead of reapplying the circuit for each observable. For simple RX/RY/RZ/CRX/CRY/CRZ and decomposition-backed `p`/`cp`/`rxx`/`ryy`/`rzz` parameter sweeps with Pauli-style observables, it can also allocate a batched simulator state and evaluate each broadcasted observable through `expectation_pauli_string_batch()`. The estimator helper layer can evaluate dense observable plans through `expectation_matrix_batch()`, but current Qiskit `EstimatorV2` pub coercion still rejects `qiskit.quantum_info.Operator` as a primitive observable.
