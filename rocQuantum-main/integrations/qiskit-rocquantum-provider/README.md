...
- **Basic Dynamic Sampling**: Executes simple Qiskit `if_test` / `if_else` conditioned blocks and `switch_case` blocks in `backend.run(..., sampling=True)` by using a state-collapsing `QuantumSimulator.measure_qubit()` trajectory per shot. Statevector and estimator outputs still reject dynamic circuits.
...
- Qiskit `if_test` / `if_else` blocks with tuple-style classical conditions and `switch_case` blocks over classical bits/registers can run in the same shot-trajectory sampling mode after prior measurements. Loop/break/continue control-flow operations, statevector output, and estimator expectation for dynamic circuits are still rejected explicitly.
