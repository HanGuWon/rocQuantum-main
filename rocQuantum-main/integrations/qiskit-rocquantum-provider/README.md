...
- `ccx`, general all-one-control `mcx`, and `cswap` are routed to native multi-control public simulator gates when available. `ch`, `cy`, `ccz`, and `dcx` use exact native decompositions. Other relative-phase controlled gates such as `rccx` / `rcccx` remain matrix fallbacks.
