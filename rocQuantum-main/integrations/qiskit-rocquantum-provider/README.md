...
- **Matrix Fallback Gates**: Exposes common Qiskit matrix gates including `sx`, `sxdg`, `rxx`, `ryy`, `rzz`, and `u` through rocQuantum matrix application.
...
- Common one- and two-qubit matrix gates (`sx`, `sxdg`, `rxx`, `ryy`, `rzz`, `u`) are target-visible and execute through `apply_matrix()`. Qiskit `p` and `cp` are target-visible but prefer exact native decomposition; statevector-producing runs include the required global phase, while sampling-only and estimator runs omit it because it cannot affect observed results.
