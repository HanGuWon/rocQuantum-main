...
- **Native Controlled-Pauli Decomposition**: Dispatches Qiskit `cy` and `ccz` through exact native `s` / `sdg` / `h` / `cx` / `mcx` decompositions.
...
- **Native Single-Qubit Decomposition**: Dispatches Qiskit `sx`, `sxdg`, and `u` through exact native rotation sequences, preserving global phase only for statevector-producing runs.
...
- `ccx`, general all-one-control `mcx`, and `cswap` are routed to native multi-control public simulator gates when available. `cy` and `ccz` use exact native decompositions. Other relative-phase controlled gates such as `rccx` / `rcccx` remain matrix fallbacks.
