...
- **Native Toffoli / Fredkin Aliases**: Dispatches `qml.Toffoli` and `qml.CSWAP` through rocQuantum native `MCX` / `CSWAP` gate aliases when the binding supports them, while keeping generic `qml.MultiControlledX` on the matrix fallback path for non-default control values.
