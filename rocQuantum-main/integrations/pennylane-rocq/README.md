...
- **Native Controlled-Pauli Decomposition**: Dispatches `qml.CY` and `qml.CCZ` through exact native `S` / `SDG` / `H` / `CNOT` / `MCX` decompositions.
...
- **GPU Matrix Fallbacks**: Dispatches common PennyLane matrix-defined gates such as `CH`, non-default-control `MultiControlledX`, `IsingXY`, swap-family gates, excitation gates, `OrbitalRotation`, and `FermionicSWAP` through rocQuantum matrix application instead of decomposing them into longer gate sequences.
