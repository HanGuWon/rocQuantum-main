...
- The current device is a legacy `QubitDevice` adapter behind PennyLane's compatibility facade. It exposes `qml.state()`, full-wire and marginal `qml.probs()`, finite-shot `qml.sample()` / `qml.counts()` including shot vectors through `QuantumSimulator.measure()`, native analytic Pauli and Pauli-Hamiltonian expectation / variance through `QuantumSimulator.expectation_pauli_string()`, direct native dispatch for `RX` / `RY` / `RZ`, `CRX` / `CRY` / `CRZ`, `Rot` decomposition, initial `qml.BasisState`, and matrix fallback dispatch for common one-, two-, and three-qubit PennyLane gates, with statevector fallbacks for older bindings.
