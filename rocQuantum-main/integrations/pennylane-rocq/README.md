...
- **Native Select Decomposition**: Dispatches full `qml.Select` blocks through native controlled single-qubit operations, selected `BasisEmbedding` products used by `qml.QROM`, or the small controlled-matrix hook for selected target operators, with batched selected rotation angles when compatible.
- **Block-Encoding Template Coverage**: Verified PennyLane-expanded `qml.QROM` and `qml.FABLE` template paths avoid dense full-template matrix fallback for the covered shapes by lowering to native `CNOT` / `MCX` / `CSWAP` / rotation / swap sequences.
