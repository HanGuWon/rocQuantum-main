...
- The current device is a legacy `QubitDevice` adapter behind PennyLane's compatibility facade. It exposes `qml.state()`, native sampling through `QuantumSimulator.measure()`, native analytic Pauli and Pauli-Hamiltonian expectation through `QuantumSimulator.expectation_pauli_string()`, direct native dispatch for `RX` / `RY` / `RZ` and `Rot` decomposition, and statevector fallbacks for older bindings.
