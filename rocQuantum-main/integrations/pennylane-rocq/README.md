...
- **Native Single-Excitation Decomposition**: Dispatches `qml.SingleExcitation` through exact native `H` / `CNOT` / `RY` decomposition instead of a dense two-qubit matrix.
- **GPU Matrix Fallbacks**: Dispatches common PennyLane matrix-defined gates such as non-default-control `MultiControlledX`, `DoubleExcitation`, `OrbitalRotation`, and `FermionicSWAP` through rocQuantum matrix application instead of decomposing them into longer gate sequences.
