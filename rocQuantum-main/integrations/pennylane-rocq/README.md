...
- **Block-Encoding Template Coverage**: Verified PennyLane-expanded `qml.QROM`, positive-coefficient `qml.PrepSelPrep`, and `qml.FABLE` template paths avoid dense full-template matrix fallback for the covered shapes by lowering to native `CNOT` / `MCX` / `CSWAP` / rotation / swap sequences.
