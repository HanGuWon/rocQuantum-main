...
- **BlockEncode Matrix Dispatch**: Accepts dense `qml.BlockEncode` operations through rocQuantum matrix application and keeps fixed `BlockEncode` blocks inside compatible PennyLane batch executions. Sparse `BlockEncode` inputs are currently materialized to a dense unitary for dispatch; a true sparse gate-application path is still pending.
...
- **GPU Matrix Fallbacks**: Dispatches remaining PennyLane matrix-defined gates such as explicit unitary operations and dense/materialized `qml.BlockEncode` through rocQuantum matrix application instead of rejecting them.
