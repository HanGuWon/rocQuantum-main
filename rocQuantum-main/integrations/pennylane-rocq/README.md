...
- **BlockEncode Matrix Dispatch**: Accepts dense `qml.BlockEncode` operations through rocQuantum matrix application and keeps fixed `BlockEncode` blocks inside compatible PennyLane batch executions. Sparse `BlockEncode` inputs use a sparse statevector fallback before dense materialization; a native sparse GPU gate-application path is still pending.
