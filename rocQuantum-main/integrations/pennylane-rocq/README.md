...
- **GPU Matrix Fallbacks**: Dispatches common PennyLane matrix-defined gates such as `PhaseShift`, controlled phase variants, `CH` / `CY` / `CCZ`, non-default-control `MultiControlledX`, Ising gates, swap-family gates, excitation gates, `OrbitalRotation`, and `FermionicSWAP` through rocQuantum matrix application instead of decomposing them into longer gate sequences.
