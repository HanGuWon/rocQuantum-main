...
- PennyLane parameter-shift gradients can run through the plugin for supported parametric gates and Pauli expectations. Simple `batch_execute` parameter sweeps use rocQuantum batch state paths for native rotations, controlled rotations, `Rot` / `CRot` decompositions, `PhaseShift`, `ControlledPhaseShift`, and `IsingXX`/`IsingYY`/`IsingZZ` decompositions, including analytic circuits that return multiple Pauli expectation values, Hermitian expectation values, variances, or probability readouts. Fixed native decompositions in the same tapes, such as `CH`, `CY`, `CCZ`, `ECR`, `ISWAP`, excitation gates, and fermionic/orbital rotations, remain in the batched execution path when their parameters are identical across the batch. Explicit `diff_method="adjoint"` is available through PennyLane's legacy adjoint algorithm by capturing the rocQuantum post-forward state and applying the reverse sweep with NumPy tensor contractions; this reduces repeated forward executions for supported analytic expectation-value circuits, but is not yet equivalent to Lightning.GPU's native GPU-resident adjoint differentiation path.
