...
- **Batched Hermitian Readouts**: Simple parameter sweeps over `qml.Hermitian` expval/variance can use `QuantumSimulator.expectation_matrix_batch()` instead of replaying each circuit when the binding exposes the dense batch hook. Distributed dense-matrix expectation has an explicit slow/debug host fallback.
