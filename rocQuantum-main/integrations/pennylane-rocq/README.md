...
- **Batched Hermitian Readouts**: Simple parameter sweeps over `qml.Hermitian` expval/variance, including scalar `qml.s_prod` / arithmetic wrappers, can use `QuantumSimulator.expectation_matrix_batch()` and `expectation_matrix_moments_batch()` instead of replaying each circuit when the binding exposes the dense batch hooks; scalar identity Hermitian matrices are folded into constant Pauli payloads without dense expectation hooks or statevector readback. Larger dense Hermitian readouts and distributed dense-matrix expectation have explicit slow/debug host fallbacks.
