...
- **Native Excitation Decomposition**: Dispatches `qml.SingleExcitation` and `qml.DoubleExcitation` through exact native `H` / `CNOT` / `RY` decompositions instead of dense two- and four-qubit matrices.
...
- **GPU Matrix Fallbacks**: Dispatches remaining PennyLane matrix-defined gates such as non-default-control `MultiControlledX` and explicit unitary operations through rocQuantum matrix application instead of rejecting them.
