...
- **Sparse Hamiltonian Moments**: Prefers the binding-level CSR sparse-Hamiltonian moments method when available, with Python statevector CSR evaluation as a compatibility fallback that avoids densifying the observable.
...
- `QuantumSimulator.sparse_hamiltonian_moments()` is currently a CSR, non-densifying binding-level implementation over a statevector readback. It is the stable framework hook for replacing this path with a validated ROCm sparse-observable GPU kernel.
