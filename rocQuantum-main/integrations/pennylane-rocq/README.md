...
- **Initial State Preparation**: Supports initial `qml.BasisState` through native `X` gates and initial `qml.StatePrep` through matrix dispatch.
...
- **Native Multi-control Aliases**: Dispatches all-one-control `qml.MultiControlledX`, `qml.Toffoli`, and `qml.CSWAP` through rocQuantum native `MCX` / `CSWAP` gate aliases when the binding supports them, while keeping non-default `MultiControlledX(control_values=...)` on the matrix fallback path.
...
- **Native Ising Interaction Decomposition**: Dispatches `qml.IsingXX`, `qml.IsingYY`, and `qml.IsingZZ` through exact native `RX` / `RZ` / `CNOT` sequences instead of dense two-qubit matrices.
- **Native Swap Decomposition**: Dispatches `qml.ISWAP` and `qml.PSWAP` through exact native `S` / `H` / `SWAP` / `CNOT` / phase decompositions instead of dense two-qubit matrices.
...
- **GPU Matrix Fallbacks**: Dispatches common PennyLane matrix-defined gates such as non-default-control `MultiControlledX`, `IsingXY`, remaining swap-family gates, excitation gates, `OrbitalRotation`, and `FermionicSWAP` through rocQuantum matrix application instead of decomposing them into longer gate sequences.
...
- Initial `qml.StatePrep` uses PennyLane's preparation matrix because a native arbitrary statevector setter is still not exposed by the rocQuantum binding. Mid-circuit state overwrite is rejected until runtime-level non-unitary state reset semantics exist.
