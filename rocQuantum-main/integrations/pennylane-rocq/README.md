...
- **Dense BlockEncode Dispatch**: Accepts dense `qml.BlockEncode` operations through rocQuantum matrix application and keeps fixed `BlockEncode` blocks inside compatible PennyLane batch executions. Sparse `BlockEncode` matrices still require a future sparse operator path.
- **Sparse Hamiltonian Moments**: Prefers native CSR sparse-Hamiltonian moments through `rocsvGetSparseMatrixMoments` / `rocsvGetSparseMatrixMomentsBatch` when available, with Python statevector CSR evaluation as a compatibility fallback that avoids densifying the observable.
- **Batched Hermitian Readouts**: Simple parameter sweeps over `qml.Hermitian` expval/variance can use `QuantumSimulator.expectation_matrix_batch()` instead of replaying each circuit when the binding exposes the dense batch hook.
...
- **GPU Matrix Fallbacks**: Dispatches remaining dense PennyLane matrix-defined gates such as explicit unitary operations and `qml.BlockEncode` through rocQuantum matrix application instead of rejecting them.
...
- `lightning.rocq` / `lightning.rocm` are compatibility entry points intended to mirror the user experience of PennyLane Lightning devices on AMD GPUs. They currently share the rocQuantum `QubitDevice` adapter; full Lightning.GPU parity still requires native HIP adjoint differentiation, batching, and ROCm E2E performance validation.
- The current device is a legacy `QubitDevice` adapter behind PennyLane's compatibility facade. It exposes `qml.state()`, full-wire and marginal `qml.probs()` through a native probability hook when available or cached statevector fallback otherwise, finite-shot `qml.sample()` / `qml.counts()` including shot vectors through `QuantumSimulator.measure()`, native analytic Pauli, Hadamard, computational-basis Projector, and Pauli-Hamiltonian expectation / variance through `QuantumSimulator.expectation_pauli_string()` without full statevector readback, analytic `qml.SparseHamiltonian` expectation / variance through native CSR moments, including simple batch parameter sweeps, when available or a statevector CSR fallback otherwise, direct native dispatch for `RX` / `RY` / `RZ`, `CRX` / `CRY` / `CRZ`, `qml.CRot`, `qml.MultiControlledX` including open controls, `qml.ControlledQubitUnitary` via native controlled-matrix dispatch when exposed, dense `qml.BlockEncode` via matrix dispatch, `qml.ControlledSequence` for native single-qubit bases, full and partial `qml.Select` blocks for supported selected operations including PennyLane-expanded `qml.QROM` selected `BasisEmbedding` products and signed/complex-coefficient `qml.PrepSelPrep` selected Pauli/global-phase products, `qml.Toffoli`, `qml.CSWAP`, `qml.QubitSum`, `qml.QubitCarry`, `qml.GroverOperator`, `qml.BasisEmbedding`, `qml.Permute`, `qml.CH`, `qml.CY`, `qml.CCZ`, `qml.ECR`, `qml.ISWAP`, `qml.PSWAP`, `qml.SISWAP`, `qml.SQISW`, `qml.SingleExcitation`, `qml.SingleExcitationPlus`, `qml.SingleExcitationMinus`, `qml.DoubleExcitation`, `qml.DoubleExcitationPlus`, `qml.DoubleExcitationMinus`, `qml.FermionicSWAP`, `qml.OrbitalRotation`, `qml.MultiRZ`, `qml.PauliRot`, `qml.SelectPauliRot`, `qml.DiagonalQubitUnitary`, `qml.GlobalPhase`, `qml.QFT`, and `qml.IsingXX/YY/XY/ZZ` via native CNOT/rotation/controlled-phase decompositions, `qml.PhaseShift` / controlled-phase variants via exact phase decompositions with measurement-aware global-phase elision, `Rot` decomposition, initial `qml.BasisState`, full-wire initial `qml.StatePrep` via native statevector upload, framework-expanded `qml.FABLE` via native rotations and swaps for tested shapes, and matrix fallback dispatch for remaining common one-, two-, and three-/four-qubit PennyLane gates, with statevector fallbacks for older bindings and arbitrary Hermitian observables.
- `QuantumSimulator.sparse_hamiltonian_moments()` / `sparse_hamiltonian_moments_batch()` upload CSR buffers and call `rocsvGetSparseMatrixMoments` / `rocsvGetSparseMatrixMomentsBatch` for native HIP row-wise sparse moments reductions on local single-state and batched state vectors. Distributed sparse moments and AMD GPU performance validation are still pending.
...
- PennyLane parameter-shift gradients can run through the plugin for supported parametric gates and Pauli expectations. Simple `batch_execute` parameter sweeps use rocQuantum batch state paths for native rotations, controlled rotations, `Rot` / `CRot` / `ControlledSequence` / `Select` decompositions, `DiagonalQubitUnitary`, `SelectPauliRot`, `GlobalPhase` elision, `PhaseShift`, `ControlledPhaseShift`, open-control `CPhaseShift00/01/10`, `MultiRZ`, `PauliRot`, `IsingXX`/`IsingYY`/`IsingZZ`/`IsingXY`, `PSWAP`, `SingleExcitation` plus/minus variants, `DoubleExcitation` plus/minus variants, `FermionicSWAP`, and `OrbitalRotation` decompositions, including analytic circuits that return multiple Pauli expectation values, Hermitian expectation values, variances, or probability readouts. Initial full-wire `StatePrep`, identical initial `BasisState` preparation, fixed `QubitUnitary` / `ControlledQubitUnitary`, and fixed native decompositions in the same tapes, such as `QFT`, `QubitSum`, `QubitCarry`, `GroverOperator`, `BasisEmbedding`, `Permute`, `CH`, `CY`, `CCZ`, `ECR`, and `ISWAP`, remain in the batched execution path when compatible across the batch. Explicit `diff_method="adjoint"` is available through PennyLane's legacy adjoint algorithm by capturing the rocQuantum post-forward state and applying the reverse sweep with NumPy tensor contractions; this reduces repeated forward executions for supported analytic expectation-value circuits, but is not yet equivalent to Lightning.GPU's native GPU-resident adjoint differentiation path.
