...
- **Lightning-style Plugin Aliases**: Registers `lightning.rocq` and `lightning.rocm` in addition to `rocquantum.qpu` and the historical `rocq.pennylane` alias, while avoiding a conflicting `lightning.amdgpu` entry point that is owned by PennyLane Lightning.
...
- **Native Controlled Sequence Decomposition**: Dispatches `qml.ControlledSequence` for native single-qubit bases through controlled powers using `CRX` / `CRY` / `CRZ` / controlled-phase / controlled-Pauli decompositions, including batched rotation-angle sweeps and adjoint payload lowering for fixed bases plus trainable scalar rotation/phase bases.
- **Native Select Decomposition**: Dispatches full and partial `qml.Select` blocks through native controlled single-qubit operations, selected `BasisEmbedding` products used by `qml.QROM`, or the small controlled-matrix hook for selected target operators, with batched selected rotation angles when compatible and primitive adjoint payload lowering for covered selected operations, fixed selected `BasisEmbedding`, simple selected basis/phase or multi-native products, and fixed selected matrix operations.
...
- **Sparse Hamiltonian Moments**: Prefers native CSR sparse-Hamiltonian moments through `rocsvGetSparseMatrixMoments` / `rocsvGetSparseMatrixMomentsBatch` when available, including scalar `qml.s_prod` / arithmetic wrappers by normalizing CSR data for prepared-state cache reuse and reapplying the scalar to mean/moment results; scalar identity and small diagonal CSR observables are folded into constant or Pauli-Z payloads without CSR moments or statevector readback. Python statevector CSR evaluation remains a compatibility fallback that avoids densifying the observable. Distributed sparse moments have an explicit slow/debug host fallback for full-state CSR observables.
- **Projector Readouts**: Computational-basis `qml.Projector` expectation and variance expand to Pauli-Z product terms without statevector readback; bare and scalar-wrapped Projector variance use the projector idempotence relation `P^2=P`, including compatible `batch_execute`, instead of building a separate Pauli-square readout plan.
- **Hermitian Readouts**: Single analytic execution and simple parameter sweeps over `qml.Hermitian` expval/variance, including scalar `qml.s_prod` / arithmetic wrappers, can use `QuantumSimulator.expectation_matrix()` / `expectation_matrix_batch()` and moment hooks while normalizing scalar-multiple dense readouts for cache reuse; scalar identity and small diagonal Hermitian matrices are folded into constant or Pauli-Z payloads without dense expectation hooks or statevector readback. Larger dense Hermitian readouts and distributed dense-matrix expectation have explicit slow/debug host fallbacks.
- **Mixed Observable Readouts**: Expectation sums that combine Pauli-representable terms with dense `qml.Hermitian` or CSR `qml.SparseHamiltonian` terms use native component readouts for analytic expval and compatible `batch_execute`; compatible batches share sum-component readouts with top-level measurements in the same prepared-state cache, dense-only mixed variance can use dense matrix moment hooks for small target sets, mixed variance containing `SparseHamiltonian` uses PennyLane's CSR observable representation with native sparse moments, and explicit adjoint payloads can carry heterogeneous Pauli, dense-matrix, and sparse-CSR terms for one observable.
...
- **Native Diagonal-Unitary Decomposition**: Dispatches `qml.DiagonalQubitUnitary` through recursive global-phase / `RZ` / uniformly controlled-`RZ` decompositions, batches varying diagonal phases by binding only the generated `RZ` angles, and keeps fixed diagonal unitaries inside native adjoint payloads around supported trainable operations.
