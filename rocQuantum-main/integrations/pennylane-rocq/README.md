...
- **Native General Controlled Wrapper Dispatch**: Accepts PennyLane `qml.ctrl(...)` wrappers for multi-controlled single-qubit `PauliX` / `PauliY` / `PauliZ` / `Hadamard` / `S` / `T` bases, lowering Pauli/Hadamard cases through native `MCX`-backed decompositions and phase-root cases through the small controlled-matrix hook instead of PennyLane-expanded local matrix fallback; fixed compatible wrappers stay inside `batch_execute`.
