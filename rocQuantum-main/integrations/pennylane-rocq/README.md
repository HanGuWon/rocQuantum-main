...
- **Batched Hermitian Readouts**: Simple parameter sweeps over `qml.Hermitian` expval/variance can use `QuantumSimulator.expectation_matrix_batch()` instead of replaying each circuit when the binding exposes the dense batch hook. Larger dense Hermitian readouts and distributed dense-matrix expectation have explicit slow/debug host fallbacks.
