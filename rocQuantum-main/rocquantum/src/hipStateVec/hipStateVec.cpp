...
inline rocqStatus_t distributed_sample_rccl(rocsvInternalHandle* handle,
    ...
    std::vector<double*> rank_probs(rank_count, nullptr);
    ...
    auto cleanup = [&]() {
        for (int rank = 0; rank < ranks; ++rank) {
            ...
            if (rank_probs[idx]) {
                (void)device_free(handle, rank_probs[idx]);
            }
    ...
    for (int rank = 0; rank < ranks; ++rank) {
        ...
        rank_probs[idx] = static_cast<double*>(probs_void);
        if (hipMemsetAsync(rank_probs[idx], 0, num_outcomes * sizeof(double), stream) != hipSuccess) {
            cleanup();
            return ROCQ_STATUS_HIP_ERROR;
        }
    ...
    rocqStatus_t status = rccl_allreduce_double_sum_inplace(handle, rank_probs, num_outcomes);
    if (status != ROCQ_STATUS_SUCCESS) {
        cleanup();
        return status;
    }
...
rocqStatus_t rocsvApplyCNOT(rocsvHandle_t handle,
    ...
    if (uses_distributed_state(handle, d_state)) {
        ...
        if (!distributed_qubit_local(handle, controlQubit) ||
            !distributed_qubit_local(handle, targetQubit)) {
            const std::vector<rocComplex> x_matrix = {
                make_complex(0.0, 0.0), make_complex(1.0, 0.0),
                make_complex(1.0, 0.0), make_complex(0.0, 0.0),
            };
            return apply_matrix_distributed_host_fallback(handle,
                                                          numQubits,
                                                          {targetQubit},
...
rocqStatus_t rocsvApplyCZ(rocsvHandle_t handle,
    ...
    if (uses_distributed_state(handle, d_state)) {
        ...
        if (!distributed_qubit_local(handle, controlQubit) ||
            !distributed_qubit_local(handle, targetQubit)) {
            const std::vector<rocComplex> z_matrix = {
                make_complex(1.0, 0.0), make_complex(0.0, 0.0),
                make_complex(0.0, 0.0), make_complex(-1.0, 0.0),
            };
            return apply_matrix_distributed_host_fallback(handle,
