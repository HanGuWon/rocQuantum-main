...
| Sampling probabilities over measured qubits | RCCL can swap-localize non-local measured qubits, sum per-rank probabilities, restore layout, and sample from the reduced distribution | `distributed_sample_rccl` / `accumulate_distributed_sample_probabilities_rccl_localized` using `rocsvSwapIndexBits` plus RCCL reductions | Host fallback after `ROCQ_DISTRIBUTED_FALLBACK_MODE=host` when localization or RCCL is unavailable | Correctness path for covered single-node layouts, not a full distributed sampling product claim |
...
| Sampling | Slice-domain measured qubits require temporary swap-localization and enough unused local slots; broader layouts may still be unsupported | RCCL path attempts swap-localization first; explicit host fallback can provide slow correctness when enabled |
