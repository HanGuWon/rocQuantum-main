# Evidence Index (baseline `5aaba9d35e783814a6abf4f6057556a4b8d8e3ac`)

## CI / Build / Validation
- `.github/workflows/rocm-linux-build.yml`: ROCm compile uses container matrix (`6.2.2`, experimental latest-production `7.2.4`).
...
- `.github/workflows/rocm-linux-build.yml`: ROCm container matrix (`6.2.2`, `7.2.4`).
...
- `VALIDATION_RESULTS.md`: local Python 3.11.15 unit and adapter tests passed; native ROCm validation remains blocked by missing local HIP toolchain/runtime.
...
- `docker/rocm/Dockerfile:1`-`26`: reproducible ROCm container lane with build-arg-selected ROCm version and pinned Python tooling.
